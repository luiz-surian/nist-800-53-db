...
--description
"A organização:
a. Desenvolve, documenta e dissemina para [Atribuição: funcionários ou departamentos definidos pela organização]:
1. Uma política de controle de acesso que lide com propósito, escopo, funções, responsabilidades, comprometimento de gerenciamento, coordenação entre entidades organizacionais e conformidade; e
2. Procedimentos para facilitar a implementação da política e controles de acesso associados; e
b. Revisa e atualiza a corrente:
...
d. Especifica usuários autorizados do sistema de informações, membros de grupos e funções e permissões de acesso (ou seja, privilégios) e outros atributos (conforme necessário) para cada conta;
e. Requer aprovações por [Atribuição: funcionários ou departamentos definidos pela organização] para solicitações de criação de contas do sistema de informações;
