...
O poder de processamento de um dispositivo móvel pode ser comparado ao de um computador desktop dependendo das funções desempenhadas por ele.
...
"A organização deverá considerar qual o tipo de auditoria que deverá ocorrer e se planejar quanto ao espaço de armazenamento disponível para guardar esses registros.
Alocar espaço suficiente reduz o risco dos registros deixarem de ser efetuados por falta de espaço, impactando na redução da capacidade da auditoria ou sua perda total."
),
(30, 2, 5, 1, 1, --AU-5
...
"Falhas nos processos de auditoria incluem, por exemplo, erros de software e/ou hardware, falhas nos mecanismos de captura da auditoria, capacidade de armazenamento lotada.
A organização poderá escolher ações adicionais por fatores diferentes de falha ao invés de uma regra global (por exemplo, por tipo, por local, por gravidade, ou uma combinação de fatores).
Esse controle pode ser aplicado para cada repositório de armazenamento de auditoria individualmente (ou seja, componentes distintos de sistemas aonde registros de auditoria são armazenados), para a capacidade total de armazenamento da organização (ou seja, todos os sistemas combinados), ou ambos."
),
(31, 2, 6, 1, 1, --AU-6
...
"Revisão, análise e relatórios de auditoria abrange registros de segurança da informação realizada por organizações, incluindo, por exemplo, registros de monitoramento de uso de conta, acesso remoto, conectividade sem fio, conexão por dispositivo móvel, configurações, inventário de componentes do sistema, uso de ferramentas de manutenção, acesso físico, temperatura e umidade, entrega e remoção de equipamentos, comunicações nos limites do sistema de informação, uso de rede móvel (GSM) e uso de VoIP.
Os resultados podem ser informados para entidades organizacionais que incluem, por exemplo, equipe de resposta a incidentes, help desk, grupo e/ou departamento de segurança da informação.
Se as organizações forem proibidas de analisar e analisar informações de auditoria ou não puderem realizar tais atividades (por exemplo, em determinados aplicativos ou sistemas de segurança nacional), a revisão e análise poderão ser realizadas por outras organizações que possuam tal autoridade."
),
(32, 2, 7, 2, 2, --AU-7
...
"O resumo de auditoria é um processo que manipula as informações dos registros de auditoria coletados e os organiza em um formato resumido que é mais compreensível para os analistas.
Os recursos que efetuam o resumo de auditoria e geração de relatórios não precisam obrigatoriamente ser do mesmo sistema ou organização.
O recurso de resumo de auditoria pode incluir, por exemplo, técnicas modernas de mineração de dados com filtros de dados avançados, como computação neural (machine learning), para identificar comportamentos incomuns nos registros da auditoria.
O sistema poderá permitir personalizar os relatórios de acordo com as necessidades do analista.
O sistema deve prever que a ordenação dos registros de auditoria por horário pode ser um problema significativo se a granularidade do registro de data e hora no registro for insuficiente."
),
(33, 2, 8, 1, 1, --AU-8
"MARCAÇÃO DE TEMPO", 
"O sistema:
...
"Marcações de tempo devem incluir data e hora.
O tempo pode ser expressado pelo uso de UTC (Coordinated Universal Time), GMT (Greenwich Mean Time), ou fuso horário local com base em UTC com compensação de horário, como exemplo, o fuso horário de brasília se baseia no UTC com subtração de três horas (UTC-3).
A granularidade da medição de tempo se refere ao sincronismo entre os relógios do ambiente operacional e ao de referência, por exemplo, pode estar sincronizado pelos segundos ou milissegundos. A organização poderá definir níveis diferentes de granularidade por cada componente do sistema se for necessário.
A granularidade do tempo escolhida poderá ser crítica a outros sistemas com capacidades de segurança como controle de acesso, identificação e autorização, dependendo dos mecanismos utilizados para suportar essas capacidades."
),
(34, 2, 9, 1, 1, --AU-9
...
"Os registros de auditoria incluem todas as informações necessárias para auditar com êxito a atividade do sistema de informações.
Esse controle se concentra na proteção lógica das informações de auditoria. A proteção física das informações de auditoria é abordada pelos controles de proteção de mídia e físicos e controles de proteção ambiental."
),
(35, 2, 10, 2, 3, --AU-10
"NÃO REPÚDIO",
...
"Tipos de ações individuais cobertas por não-repúdio incluem, por exemplo, criar informações, enviar e receber mensagens, aprovar informações (por exemplo, indicando concordância ou assinar um contrato).
O não-repúdio protege indivíduos contra reivindicações posteriores por:
I. autores de uma ação e/ou documento;
II. remetentes de não terem transmitido mensagens;
III. destinatários não terem recebido mensagens; ou
IV. usuários não ter assinado documentos.
Os serviços de não repúdio podem ser usados para determinar se as informações foram originadas de um indivíduo em particular ou se um indivíduo realizou ações específicas (por exemplo, enviar um e-mail, assinar um contrato, aprovar uma solicitação de aquisição) ou receber informações específicas.
A organização deve obter serviços de não repúdio empregando várias técnicas ou mecanismos (por exemplo, assinaturas digitais, recibos de mensagens digitais)."
),
(36, 2, 11, 3, 1, --AU-11
