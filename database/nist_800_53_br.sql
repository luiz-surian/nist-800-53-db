...
"Esse controle aborda a criação de políticas e procedimentos para a implementação efetiva de controles de segurança e aprimoramentos na família AC. As políticas e procedimentos refletem as leis federais, ordens executivas, diretivas, regulamentos, políticas, normas e diretrizes. Políticas e procedimentos de segurança a nível organizacional podem substituir a necessidade de políticas e procedimentos específicos por sistema. A política pode fazer parte da política geral de segurança da informação organizacional ou, poderá0 ser representada por múltiplas políticas individuais. Os procedimentos podem ser estabelecidos para o programa de segurança organizacional e/ou para sistemas de informação específicos, se necessário. A estratégia de gerenciamento de risco organizacional é um fator chave no estabelecimento das políticas e procedimentos."
),
(2, 1, 2, 1, 1, --AC-2
"GESTÃO DE CONTA",
"A organização:
...
"Os tipos de conta do sistema de informações são: individual, compartilhado, grupo, sistema, convidado / anônimo, emergência, desenvolvedor / fabricante / fornecedor, temporário e serviço. Alguns dos requisitos de gerenciamento de contas listados acima podem ser implementados pelos sistemas de informações organizacionais. A identificação de usuários autorizados do sistema de informações e a especificação de privilégios de acesso refletem os requisitos em outros controles de segurança no plano de segurança. Os usuários que exigem privilégios administrativos em contas do sistema de informações recebem uma análise adicional por parte do pessoal organizacional apropriado (por exemplo, proprietário do sistema, gerente departamental ou diretor de segurança da informação) responsável pela aprovação de tais contas e acesso privilegiado. As organizações podem optar por definir privilégios de acesso e atributos: por conta, por tipo de conta ou por uma combinação de ambos. Outros atributos necessários para autorizar o acesso incluem, por exemplo, restrições na hora do dia, no dia da semana e no local de acesso. Na definição de outros atributos de conta, as organizações consideram os requisitos relacionados ao sistema (por exemplo, manutenção programada, atualizações do sistema) e requisitos de missão / negócios (por exemplo, diferenças de fuso horário, requisitos do cliente, acesso remoto para atender aos requisitos de viagem). Não considerar esses fatores pode afetar a disponibilidade do sistema de informação. Contas temporárias e de emergência são contas destinadas a uso de curto prazo. A organização pode estabelecer contas temporárias como parte dos procedimentos normais de ativação de contas quando há necessidade de contas de curto prazo sem a demanda por imediatismo na ativação da conta. A organização pode estabelecer contas de emergência em resposta a situações de crise e com a necessidade de ativação rápida de conta. Portanto, a ativação de conta de emergência pode ignorar os processos normais de autorização de conta. As contas de emergência e temporárias não devem ser confundidas com contas utilizadas com pouca frequência (por exemplo, contas de logon locais usadas para tarefas especiais definidas pela organização ou quando os recursos de rede não estão disponíveis). Essas contas permanecem disponíveis e não estão sujeitas a datas automáticas de desativação ou remoção. As condições para desabilitar ou desativar contas incluem, por exemplo: I. quando contas compartilhadas / de grupo, de emergência ou temporárias não são mais necessárias; ou II. quando os indivíduos são transferidos ou terminados. Alguns tipos de contas do sistema de informações podem exigir treinamento especializado."
),
(3, 1, 3, 1, 1, --AC-3
"GARANTIA DE ACESSO",
...
"Segregação de funções se refere ao potencial abuso de privilégios de acesso e ajuda a reduzir o risco de ações mal-intencionadas sem consequências. A segregação inclui, por exemplo: I. Dividir acesso e funções do sistema entre diferentes funcionários e/ou departamentos; II. Atribuir funções de suporte do sistema a diferentes usuários (Por exemplo, controle do sistema, programação, controle de configurações, testes de qualidade e segurança de rede); e III. Garante que funcionários da segurança que administram as funções de controle de acesso não administrem as auditorias."
),
(6, 1, 6, 1, 2, --AC-6
"MENOR PRIVILÉGIO",
...
"O princípio do menor privilégio também deverá ser aplicado aos processos usados pelos sistemas de informação, garantindo que esses processos não possuam permissões mais do que as necessárias para garantir seu funcionamento. A organização poderá criar novos processos, funções, grupos e usuários de acordo com o que julgar necessário para atingir o menor privilégio possível por aplicação/usuário. O princípio do menor privilégio deverá ser adotado ao desenvolvimento, implementação e operação dos sistemas de informação. O uso de contas Administrativas, Root, Sudo, devem ser evitadas ao máximo."
),
(7, 1, 7, 2, 1, --AC-7
...
"Esse controle se aplica a todos os sistemas, independente se forem acessado internamente ou externamente. Para se prevenir uma potencial indisponibilidade de serviço (DoS) ou ataques de força-bruta para descobrir um login válido. Esse controle deverá ser aplicado tanto no sistema operacional quanto nas aplicações."
),
(8, 1, 8, 1, 1, --AC-8
...
"O sistema de notificação desta mensagem pode ser aplicado por telas ou baners que apareçam antes que o usuário faça o login no sistema. Esses avisos só são necessários para a interface dos sistemas que irão possui interação humana, portanto não são necessários para processos ou usuários sistêmicos. A organização deverá considerar que essa mensagem poderá necessitar ser traduzida para diferentes línguas de acordo com as regiões demográficas."
),
(9, 1, 9, 0, 0, --AC-9
...
"Bloqueios de sessão devem ser controles temporários para quando há a necessidade do usuário se afastar do sistema mas não precisa realmente efetuar logout por se ausentar temporariamente. Geralmente este controle fica a nível do sistema operacional, porém poderá ser implementado a nível de aplicação caso seja necessário. Esse controle não substitui a necessidade do usuário realmente sair do sistema, como deve ocorrer, por exemplo, ao final do expediente."
),
(12, 1, 12, 2, 2, --AC-12
"FINALIZAR SESSÃO",
"O sistema irá efetuar o logout do usuário automaticamente após [Atribuição: controles e eventos definidos pela organização].",
"Esse controle se trata da finalização do acesso lógico aos sistemas, ao contrário do controle SC-10 que se trata de comunicações da rede. Uma sessão lógica é iniciada quando um usuário (ou processo) acessa um sistema organizacional. Ao finalizar um sessão, todos os processos do sistema associados ao usuário em questão deverão ser finalizados, com exceção de processos que são criado com o propósito de persistir após o logout. Para o evento de finalizar sessão ocorrer poderá ser por pedido do usuário, período de inatividade, incidentes e/ou restrições de dias e horários."
),
(13, 1, 13, 0, 0, --AC-13
...
"Esse controle se refere a situações onde a organização necessita que determinadas ações possam ocorrer no sistema sem uma identificação ou autenticação prévia por parte do usuário, como exemplo, o uso do site da organização disponível publicamente. O número de ações deste tipo deverá ser limitado para somente o que for necessário."
),
(15, 1, 15, 0, 0, --AC-15
"MARCAÇÃO AUTOMÁTICA",
...
"Entende-se como acesso remoto a sistemas quando a conexão ocorre por redes externas (por exemplo, a Internet). A organização deverá implementar o uso de redes privadas virtuais encriptadas (VPN) para melhorar a integridade e confiança das informações por conexões remotas. O uso de VPN não faz com que a conexão deixe de ser remota, porém, quando devidamente aplicado com controles de segurança apropriados poderá permitir que os controles sobre o sistema sejam os mesmos aplicados à conexões internas. Mas de qualquer forma, VPNs ainda acontecem sobre conexões externas, não garantindo a disponibilidade da comunicação, e pela comunicação ser encriptada as ferramentas de controle e monitoramento contra códigos maliciosos serão afetadas. As permissões de acesso não são definidos por esse controle, e sim pelo AC-3."
),
(18, 1, 18, 1, 1, --AC-18
"ACESSO WIRELESS",
"A organização:
...
"Tecnologias sem fio são, por exemplo, radio (AM/FM), Wi-Fi (802.11x), mobile (GSM) e Bluetooth. Conexões sem fio devem utilizar protocolos de autenticação que provêm proteção a credencial e autenticação mútua."
),
(19, 1, 19, 1, 1, --AC-19
...
"Um dispositivo móvel é aquele que: I. Possui um tamanho pequeno o suficiente que permite que possa ser facilmente carregado por uma única pessoa; II. Permite conexão sem fio (wireless) para transmitir e receber informações; III. Possui espaço local de armazenamento, removível ou fixo; e IV. Possui alimentação de energia própria. Exemplos são: Notebook, Smartphones, Tablets e E-Readers. O poder de processamento de um dispositivo móvel pode ser comparado ao de um computador desktop dependendo das funções desempenhadas por ele. Controles de uso incluem, por exemplo, controle de configurações, identificação e autenticação do aparelho, implementação de software mandatório (Antivírus, Firewall, etc.), scan periódico de vulnerabilidades e códigos maliciosos, atualização frequente de seus componentes, verificações de integridade e desativar hardware desnecessário (por exemplo, Infravermelho, Bluetooth, etc.) quando não são necessários para desempenhar determinado função/negócio. Poderão existir outros controles ocorrendo ao mesmo tempo quando necessário, o controle AC-20 determina dispositivos móveis que não são da organização."
),
(20, 1, 20, 1, 1, --AC-20
...
"Sistemas externos são aqueles que a organização geralmente não possui controle direto, supervisão e autoridade sobre o sistema ou os controles de segurança atribuídos a ele. Sistemas externos incluem, por exemplo: I. Informações sobre os sistemas/dispositivos (exemplo, notebook, smartphones, tablets, assistentes digitais); II. Dispositivos presentes em áreas públicas ou comerciais (por exemplo, hotéis, estações de trem/metrô, eventos, shoppings, aeroportos, etc.); III. Sistemas de posse e controle de organizações não governamentais; e IV. Sistemas federais que não estão sobre o controle de organizações, e sim pelo governo. Esse controle também se aplica ao uso de sistemas externos para processar, armazenar e/ou transmitir informações organizacionais, por exemplo, o uso de serviços em nuvem para hospedar os sistemas. Para alguns sistemas externos, como exemplo, sistemas operados por agentes federais, uma relação de confiança já foi previamente estabelecida e portanto não necessitam termos e condições de uso explícitos. Isso ocorre quando acordos prévios de confiança e/ou compartilhamento são definidos, explicitamente ou implicitamente, estabelecidos com agências federais ou organizações subordinadas a essas agências que são aplicáveis por lei, Ordens Executivas, diretrizes ou políticas. A autorização de indivíduos inclui, por exemplo, funcionários da organização, contratados e fornecedores. As restrições sobre os tipos de acesso aos sistemas externos não precisam ser idênticos a todos os envolvidos, elas podem variar por tipo de acesso, organização, cargo, departamento, entre outros. A organização deve estabelecer termos e condições para o uso dos sistemas externos de acordo com suas próprias políticas internas de segurança e procedimentos. Os termos devem, no mínimo, apontar quais tipos de informações poderão ser acessadas nos sistemas externos. E quais podem ou não podem ser processadas, armazenadas ou transmitidas. Se esses termos não podem ser estabelecidos em conjunto com o dono do sistema, a organização deverá implementar essas restrições internamente."
),
(21, 1, 21, 2, 2, --AC-21
...
"Mineração de dados (data mining) é o processo de explorar grandes quantidades de dados à procura de padrões consistentes, como regras de associação ou sequências temporais, para obter informações privilegiadas sem prévia autorização. Objetos de armazenamento de informações incluem, por exemplo, banco de dados, tabelas, campos e documentos. Técnicas de prevenção e detecção de mineração de dados incluem, por exemplo: I. Limitar o tipo das respostas provenientes de comandos ao banco de dados; II. Limitar a quantidade e frequência de comandos ao banco de dados; III. Notificar funcionários autorizados quando comandos atípicos acontecerem ao banco de dados. Esse controle é sobre proteger e evitar que os dados sejam minerados, em contra partida, o controle AU-13 retrata quando o dado já foi minerado e está disponível a público em sites externos, como redes sociais por exemplo."
),
(24, 1, 24, 0, 0, --AC-24
...
"A informação é representada internamente em sistemas de informação usando abstrações conhecidas como estruturas de dados. Estruturas de dados internas podem representar diferentes tipos de entidades, ativas e passivas. Entidades ativas, são normalmente associadas a indivíduos, dispositivos ou processos agindo em nome de indivíduos. As entidades passivas, são normalmente associadas a estruturas de dados, como registros, buffers, tabelas, arquivos, pipes entre processos e portas de comunicação. Os monitores de referência normalmente aplicam políticas de controle de acesso obrigatório - um tipo de controle de acesso que restringe o acesso a objetos com base na identidade dos assuntos ou grupos aos quais os assuntos pertencem. Os controles de acesso são obrigatórios porque os sujeitos com certos privilégios (isto é, permissões de acesso) são impedidos de passar esses privilégios para qualquer outro assunto, direta ou indiretamente, ou seja, o sistema de informações aplica rigorosamente a política de controle de acesso com base no conjunto de regras estabelecido pela política. A propriedade inviolável do monitor de referência impede que os adversários comprometam o funcionamento do mecanismo. A propriedade sempre presente impede que os adversários contornem o mecanismo e, portanto, violem a política de segurança. A propriedade de ser pequeno ajuda a garantir a integridade na análise e teste do mecanismo para detectar deficiências ou deficiências (ou seja, falhas latentes) que impediriam a aplicação da política de segurança."
);
...
"Este controle aborda o estabelecimento de políticas e procedimentos para a implementação efetiva de controles de segurança selecionados e melhorias de controle na família AU. As políticas e procedimentos refletem as leis federais aplicáveis, ordens executivas, diretivas, regulamentos, políticas, padrões e diretrizes. Políticas e Procedimentos a nível organizacional podem substituir a necessidade de políticas e procedimentos específicos por sistemas. A política pode ser incluída como parte da política geral de segurança da informação para organizações ou, inversamente, pode ser representada por múltiplas políticas refletindo a natureza complexa da organização. Os procedimentos podem ser estabelecidos para o programa de segurança em geral e para sistemas específicos, se necessário. A estratégia de gerenciamento de risco organizacional é um fator chave no estabelecimento de políticas e procedimentos."
),
(27, 2, 2, 1, 1, --AU-2
...
"Um evento é qualquer ocorrência observável em sistemas organizacionais. A organização identifica os eventos de auditoria como aqueles que são significativos e relevantes para a segurança dos sistemas de informação e os ambientes em que esses sistemas operam para atender necessidades de auditoria específicas e contínuas. Os eventos podem incluir, por exemplo, alterações de senha, logons com falha ou falhas de acesso a sistemas de informações, uso de privilégios administrativos, uso de credenciais de PIV ou uso de credenciais de terceiros. Ao determinar o conjunto de eventos auditáveis, a organização deverá considerar a auditoria apropriada para equilibrar os requisitos de auditoria com outras necessidades do sistema de informação, esse controle também requer a identificação do subconjunto de eventos auditáveis ​​que são auditados em um determinado momento, por exemplo, a organização pode determinar que os sistemas devem ter a capacidade de registrar todos os acessos bem-sucedidos e mal-sucedidos a arquivos, exceto em circunstâncias específicas, devido à  potencial sobrecarga no desempenho do sistema. Os requisitos de auditoria, incluindo a necessidade de eventos auditáveis, podem ser referenciados em outros controles de segurança e aprimoramentos de controle. As organizações também incluem eventos auditáveis ​​que são exigidos pelas leis federais aplicáveis, ordens executivas, diretivas, políticas, regulamentos e normas. Os registros de auditoria podem ser gerados em vários níveis de abstração, inclusive no nível do pacote, à medida que as informações percorrem a rede. Selecionar o nível apropriado de abstração é um aspecto crítico de um recurso de auditoria e pode facilitar a identificação de causas-raiz para problemas. A organização deverá considerar na definição de eventos auditáveis, a auditoria necessária para cobrir eventos relacionados, como as etapas em processos distribuídos e baseados em transações (por exemplo, processos distribuídos em várias organizações) e ações que ocorrem em arquiteturas orientadas a serviços."
),
(28, 2, 3, 1, 1, --AU-3
...
"A organização disponibiliza espaço de armazenamento para registros de auditoria de acordo com [Atribuição: requerimentos sobre armazenamento de registros de auditoria definidos pela organização].",
"A organização deverá considerar qual o tipo de auditoria que deverá ocorrer e se planejar quanto ao espaço de armazenamento disponível para guardar esses registros. Alocar espaço suficiente reduz o risco dos registros deixarem de ser efetuados por falta de espaço, impactando na redução da capacidade da auditoria ou sua perda total."
),
(30, 2, 5, 1, 1, --AU-5
"RESPOSTAS ÀS FALHAS DE PROCESSOS DE AUDITORIA",
"O sistema:
a. Alerta [Atribuição: funcionários e/ou departamentos definidos pela organização] no evento de falhas no processo de auditoria; e
b. Efetua as ações: [Atribuição: ações determinadas pela organização (por exemplo, desligar o sistema, sobrescrever os arquivos antigos de auditoria, parar de efetuar registros de auditoria)].",
"Falhas nos processos de auditoria incluem, por exemplo, erros de software e/ou hardware, falhas nos mecanismos de captura da auditoria, capacidade de armazenamento lotada. A organização poderá escolher ações adicionais por fatores diferentes de falha ao invés de uma regra global (por exemplo, por tipo, por local, por gravidade, ou uma combinação de fatores). Esse controle pode ser aplicado para cada repositório de armazenamento de auditoria individualmente (ou seja, componentes distintos de sistemas aonde registros de auditoria são armazenados), para a capacidade total de armazenamento da organização (ou seja, todos os sistemas combinados), ou ambos."
),
(31, 2, 6, 1, 1, --AU-6
"REVISÃO DE AUDITORIA, ANÁLISE E RELATÓRIO",
"A organização:
a. Revisa e analisa os registros de auditoria do sistema de informação [Atribuição: frequência definida pela organização] para indicações de [Atribuição: atividade imprópria ou incomum definida pela organização]; e
b. Relata os resultados para [Atribuição: funcionários e/ou departamentos definidos pela organização].",
"Revisão, análise e relatórios de auditoria abrange registros de segurança da informação realizada por organizações, incluindo, por exemplo, registros de monitoramento de uso de conta, acesso remoto, conectividade sem fio, conexão por dispositivo móvel, configurações, inventário de componentes do sistema, uso de ferramentas de manutenção, acesso físico, temperatura e umidade, entrega e remoção de equipamentos, comunicações nos limites do sistema de informação, uso de rede móvel (GSM) e uso de VoIP. Os resultados podem ser informados para entidades organizacionais que incluem, por exemplo, equipe de resposta a incidentes, help desk, grupo e/ou departamento de segurança da informação. Se as organizações forem proibidas de analisar e analisar informações de auditoria ou não puderem realizar tais atividades (por exemplo, em determinados aplicativos ou sistemas de segurança nacional), a revisão e análise poderão ser realizadas por outras organizações que possuam tal autoridade."
),
(32, 2, 7, 2, 2, --AU-7
"RESUMO DE AUDITORIA E GERAÇÃO DE RELATÓRIO",
"O sistema provê capacidades de resumir os registros de auditoria e gerar relatórios que:
a. Permite revisões, análises e relatórios da auditoria, por demanda e em tempo real, disponíveis logo após de ocorrer um incidente de segurança; e
b. Não altera o conteúdo original ou a ordem dos registros da auditoria.",
"O resumo de auditoria é um processo que manipula as informações dos registros de auditoria coletados e os organiza em um formato resumido que é mais compreensível para os analistas. Os recursos que efetuam o resumo de auditoria e geração de relatórios não precisam obrigatoriamente ser do mesmo sistema ou organização. O recurso de resumo de auditoria pode incluir, por exemplo, técnicas modernas de mineração de dados com filtros de dados avançados, como computação neural (machine learning), para identificar comportamentos incomuns nos registros da auditoria. O sistema poderá permitir personalizar os relatórios de acordo com as necessidades do analista. O sistema deve prever que a ordenação dos registros de auditoria por horário pode ser um problema significativo se a granularidade do registro de data e hora no registro for insuficiente."
),
(33, 2, 8, 1, 1, --AU-8
"MARCAÇÃO DE TEMPO",
"O sistema:
a. Usa o relógio interno do ambiente operacional para gerar marcações de tempo (time stamps) para os registros de auditoria; e
b. Registra essas marcações de tempo que possam ser convertidas para fuso horário universal (UTC) ou fuso horário com base no meridiano de Greenwich (GMT), por exemplo, o horário oficial de Brasília é UTC-3, também deve estar de acordo com [Atribuição: granularidade da medição do tempo definida pela organização].",
"Marcações de tempo devem incluir data e hora. O tempo pode ser expressado pelo uso de UTC (Coordinated Universal Time), GMT (Greenwich Mean Time), ou fuso horário local com base em UTC com compensação de horário, como exemplo, o fuso horário de brasília se baseia no UTC com subtração de três horas (UTC-3). A granularidade da medição de tempo se refere ao sincronismo entre os relógios do ambiente operacional e ao de referência, por exemplo, pode estar sincronizado pelos segundos ou milissegundos. A organização poderá definir níveis diferentes de granularidade por cada componente do sistema se for necessário. A granularidade do tempo escolhida poderá ser crítica a outros sistemas com capacidades de segurança como controle de acesso, identificação e autorização, dependendo dos mecanismos utilizados para suportar essas capacidades."
),
(34, 2, 9, 1, 1, --AU-9
"PROTECÇÃO DAS INFORMAÇÕES DE AUDITORIA",
"O sistema deve proteger as informações dos registros de auditoria e as ferramentas de auditoria contra acesso não autorizado, modificação e exclusão.",
"Os registros de auditoria incluem todas as informações necessárias para auditar com êxito a atividade do sistema de informações. Esse controle se concentra na proteção lógica das informações de auditoria. A proteção física das informações de auditoria é abordada pelos controles de proteção de mídia e físicos e controles de proteção ambiental."
),
(35, 2, 10, 2, 3, --AU-10
"NÃO REPÚDIO",
"O sistema de informação protege contra usuários e processos falsamente negando ter realizado [Atribuição: ações definidas pela organização para ser coberto pelo não-repúdio].",
"Tipos de ações individuais cobertas por não-repúdio incluem, por exemplo, criar informações, enviar e receber mensagens, aprovar informações (por exemplo, indicando concordância ou assinar um contrato). O não-repúdio protege indivíduos contra reivindicações posteriores por: I. autores de uma ação e/ou documento; II. remetentes de não terem transmitido mensagens; III. destinatários não terem recebido mensagens; ou IV. usuários não ter assinado documentos. Os serviços de não repúdio podem ser usados para determinar se as informações foram originadas de um indivíduo em particular ou se um indivíduo realizou ações específicas (por exemplo, enviar um e-mail, assinar um contrato, aprovar uma solicitação de aquisição) ou receber informações específicas. A organização deve obter serviços de não repúdio empregando várias técnicas ou mecanismos (por exemplo, assinaturas digitais, recibos de mensagens digitais)."
),
(36, 2, 11, 3, 1, --AU-11
"RETENÇÃO DOS REGISTROS DE AUDITORIA",
"A organização retém registros de auditoria por [Atribuição: período de tempo definido pela organização consistente com a política de retenção de registros] para fornecer suporte a investigações após ocorrer incidentes de segurança e para atender aos requisitos de retenção de informações regulatórias e organizacionais.",
"As organizações retêm registros de auditoria até que seja determinado que não são mais necessários para fins administrativos, legais, de auditoria ou outros fins operacionais. Isso inclui, por exemplo, retenção e disponibilidade de registros de auditoria relativos a solicitações do governo; intimações e ações policiais. As organizações desenvolvem categorias padrão de registros de auditoria relativos a esses tipos de ações e processos de resposta padrão para cada tipo de ação."
),
(37, 2, 12, 1, 1, --AU-12
"GERAÇÃO DE AUDITORIA",
"O sistema:
a. Fornece capacidade de geração de registros de auditoria para os eventos auditáveis definidos em AU-2(a) em [Atribuição: componentes do sistema de informação definida pela organização];
b. Permite que [Atribuição: funcionários e/ou departamentos definidos pela organização] selecione quais eventos auditáveis devem ser auditados por componentes específicos do sistema; e
c. Gera registros de auditoria para os eventos definidos em AU-2(d) com o conteúdo definido em AU-3.",
"Registros de auditoria podem ser gerados a partir de diversos componentes do sistema. A lista de eventos auditados é o conjunto de eventos para os quais as auditorias serão geradas. Esses eventos são tipicamente um subconjunto de todos os eventos para os quais o sistema de informações é capaz de gerar registros de auditoria."
),
(38, 2, 13, 0, 0, --AU-13
"MONITORAMENTO PARA DIVULGAÇÃO DE INFORMAÇÕES",
"A organização monitora [Atribuição: informações de código aberto definidas pela organização e / ou sites de informações] a cada [Atribuição: frequência definida pela organização] para evidência de divulgação não autorizada de informações organizacionais.",
"Informações de código aberto incluem, por exemplo, sites de redes sociais."
),
(39, 2, 14, 0, 0, --AU-14
"AUDITORIA DA SESSÃO",
"O sistema de informação fornece a capacidade para usuários autorizados selecionarem uma sessão de usuário para capturar, gravar, visualizar e/ou ouvir.",
"Auditorias de sessão incluem, por exemplo, monitorar pressionamentos de teclas, rastrear sites visitados e registrar informações e/ou transferências de arquivos. As atividades de auditoria de sessão são desenvolvidas, integradas e usadas em consulta com o departamento jurídico de acordo com as leis federais aplicáveis, políticas, regulamentos e/ou padrões."
),
(40, 2, 15, 0, 0, --AU-15
"ALTERNATIVAS À AUDITORIA",
"A organização fornece um recurso de auditoria alternativo no caso de falha no recurso de auditoria original que fornece [Atribuição: funcionalidade de auditoria alternativa definida pela organização].",
"Como um recurso de auditoria alternativo pode ser uma proteção de curto prazo empregada até que a falha no recurso de auditoria principal seja corrigida, as organizações podem determinar que o recurso de auditoria alternativo precisa fornecer apenas uma parte da funcionalidade da auditoria original que foi afetada pela falha."
),
(41, 2, 16, 0, 0, --AU-16
"AUDITORIA ENTRE ORGANIZAÇÕES",
"A organização emprega [Atribuição: métodos definidos pela organização] para coordenar [Atribuição: informações de auditoria definidas pela organização] entre organizações externas quando as informações de auditoria são transmitidas externamente.",
"Quando as organizações usam sistemas de informação e/ou serviços de organizações externas, o recurso de auditoria exige uma abordagem coordenada entre as organizações. Por exemplo, manter a identidade de indivíduos que solicitaram determinados serviços além dos limites organizacionais pode ser muito difícil, e isso pode ter impactos significativos no desempenho. Portanto, é comum que a auditoria entre organizações (por exemplo, o tipo de recurso de auditoria fornecido por arquiteturas orientadas a serviços) simplesmente capture a identidade de indivíduos que emitem solicitações no sistema de informação inicial, e sistemas subsequentes registrem que as solicitações emanaram de indivíduos autorizados."
);
...
VALUES
(42, 3, 1, 1, 1, --AT-1
"PROCEDIMENTOS PARA CONSCIENTIZAÇÃO DE SEGURANÇA E POLÍTICA DE TREINAMENTOS",
"A organização:
a. Desenvolve, documenta, e dissemina para [Atribuição: funcionários e/ou departamentos definidos pela organização]:
1. Conscientização de segurança e política de treinamento que envolve propósito, escopo, funções, responsabilidade, comprometimento da gerência, coordenação entre entidades organizacionais e compliance; e
2. Procedimentos para facilitar a implementação da conscientização de segurança e política de treinamentos; e
b. Revisa e atualiza:
1. Conscientização de segurança e política de treinamento a cada [Atribuição: frequência definida pela organização]; e
2. Procedimentos para conscientização e treinamento de segurança a cada [Atribuição: frequência definida pela organização].",
...
"A organização efetua treinamento básico sobre conscientização de segurança para usuários de sistemas (incluindo, gestores, executivos e colaboradores):
a. Como parte do treinamento inicial para novos usuários;
b. Quando necessário por mudanças no sistema. e
c. A cada [Atribuição: frequência definida pela organização].",
"A organização deve determinar qual o conteúdo do treinamento de conscientização de segurança e quais técnicas irá aplicar de acordo com os requerimentos específicos da organização e a quais sistemas serão disponíveis aos usuários. O conteúdo possui um entendimento básico sobre a necessidade da segurança da informação e ações dos usuários para manter a segurança e como agir em suspeitas de incidentes, além de citar sobre a importância da conscientização na organização inteira. As técnicas empregadas ao treinamento incluem, por exemplo, cartazes, material de escritório com lembretes de segurança gravados neles, mensagens no login do sistema e eventos."
),
(44, 3, 3, 1, 1, --AT-3
...
a. Antes de autorizar o acesso ao sistema de informação ou executar tarefas atribuídas;
b. Quando requerido por mudanças no sistema de informação; e
c. A cada [Atribuição: frequência definida pela organização].",
"As organizações determinam o conteúdo apropriado do treinamento de segurança com base nas funções e responsabilidades atribuídas dos indivíduos e nos requisitos específicos de segurança das organizações e nos sistemas de informação para os quais o pessoal autorizou o acesso. Além disso, as organizações fornecem arquitetos empresariais, desenvolvedores de sistemas de informação, desenvolvedores de software, funcionários de aquisições / aquisições, gerentes de sistemas de informação, administradores de sistema / rede, pessoal conduzindo atividades de gerenciamento e auditoria de configuração, pessoal executando atividades de verificação e validação independentes, assessores de controle de segurança e outro pessoal que tenha acesso a software em nível de sistema, treinamento técnico adequado relacionado à segurança, especificamente adaptado para suas tarefas atribuídas. O treinamento abrangente baseado em funções aborda funções e responsabilidades técnicas, operacionais e de gerenciamento que abrangem proteções e contramedidas físicas, pessoais e técnicas. Esse treinamento pode incluir, por exemplo, políticas, procedimentos, ferramentas e artefatos para as funções de segurança organizacional definidas. As organizações também fornecem o treinamento necessário para que os indivíduos cumpram suas responsabilidades relacionadas às operações e à segurança da cadeia de fornecimento dentro do contexto dos programas de segurança da informação organizacionais. O treinamento em segurança com base em papéis também se aplica a contratados que prestam serviços a agências federais."
),
(45, 3, 4, 3, 1, --AT-4
"REGISTROS DE TREINAMENTO DE SEGURANÇA",
"A organização:
a. Documenta e monitora as atividades individualmente do treinamento de segurança do sistema, incluindo treinamento básico de conscientização de segurança e treinamento específico de segurança do sistema; e
b. Mantém registros de treinamento individuais por [Atribuição: período de tempo definido pela organização].",
...
VALUES
(47, 4, 1, 1, 1, --CM-1
"POLÍTICA E PROCEDIMENTOS DA GESTÃO DE CONFIGURAÇÃO",
"A organização:
a. Desenvolve, documenta e dissemina para [Atribuição: funcionário e/ou departamento definidos pela empresa]:
1. Uma política da gestão de configuração que trata propósito, escopo, funções, responsabilidades, comprometimento da gerência, coordenação entre entidades organizacionais e conformidade; e
2. Procedimentos que facilitam a implementação das políticas da gestão de configuração e seus controles; e
b. Revisa e atualiza:
1. As políticas da gestão de configuração a cada [Atribuição: frequência definida pela organização]; e
2. Os procedimentos da gestão de configuração a cada [Atribuição: frequência definida pela organização].",
"Esse controle aborda o estabelecimento de políticas e procedimentos para a implementação efetiva de controles de segurança selecionados e aprimoramentos de controle na família CM. As políticas e procedimentos refletem as leis federais aplicáveis, ordens executivas, diretivas, regulamentos, políticas, normas e diretrizes. Políticas e procedimentos do programa de segurança no nível da organização podem tornar desnecessária a necessidade de políticas e procedimentos específicos por sistema. A política pode ser incluída como parte da política geral de segurança da informação para a organização ou, inversamente, pode ser representada por múltiplas políticas refletindo a natureza complexa da organização. Os procedimentos podem ser estabelecidos para o programa de segurança em geral e para sistemas de informação específicos, se necessário. A estratégia de gerenciamento de risco organizacional é um fator chave no estabelecimento de políticas e procedimentos."
),
(48, 4, 2, 1, 1, --CM-2
"CONFIGURAÇÃO BASE DE REFERÊNCIA",
"A organização desenvolve, documenta e mantém sob controle de configuração, uma configuração base de referência do sistema.",
"Esse controle estabelece configurações base de referência para sistemas e seus componentes, incluindo aspectos relacionados à comunicação e à conectividade dos sistemas. As configurações de base de referência são conjuntos de especificações documentados, revisados ​​formalmente e acordados para sistemas de informação ou itens de configuração dentro desses sistemas. As configurações de base de referência servem como base para futuros builds, releases e/ou alterações nos sistemas. As configurações de base de referência incluem informações sobre componentes do sistema (por exemplo, pacotes de software padrão instalados em estações de trabalho, notebooks, servidores, componentes de rede ou dispositivos móveis; números de versão atuais e informações de patch em sistemas operacionais e aplicativos; configurações e/ou parâmetros) topologia e o posicionamento lógico desses componentes na arquitetura do sistema. A manutenção de configurações de base de referência requer a criação de novas bases à medida que os sistemas de informações organizacionais mudam com o tempo. As configurações de base de referência dos sistemas refletem a arquitetura corporativa atual."
),
(49, 4, 3, 1, 2, --CM-3
"MUDANÇAS CONTROLADAS POR CONFIGURAÇÃO",
"A organização:
a. Determina os tipos de mudanças ao sistema que são controladas por configuração;
b. Revisa as mudanças controladas por configuração no sistema e aprova ou desaprova as mudanças explicitamente para ser considerado por análises de segurança;
c. Documenta as decisões sobre a configuração de mudanças do sistema;
d. Implementa as mudanças controladas por configuração ao sistema;
e. Mantém os registros das mudanças controladas por configuração do sistema por [Atribuição: período definido pela organização];
f. Audita e revisa as atividades associadas as mudanças controladas por configuração do sistema; e
g. Coordena e supervisiona as atividades relacionadas as mudanças controladas por configuração através [Atribuição: elemento de controle definido pela organização (por exemplo, comitê, reunião, quadro de avisos)] a cada [Atribuição: frequência definida pela organização].",
"Mudanças no sistema controladas por configuração envolvem a proposta sistemática, justificativa, implementação, teste, revisão e disposição de mudanças nos sistemas, incluindo atualizações e modificações do sistema. O controle de alterações de configuração inclui alterações nas configurações de base de referência para componentes e itens de configuração de sistemas, alterações nas configurações de produtos de tecnologia da informação (por exemplo, sistemas operacionais, aplicativos, firewalls, roteadores e dispositivos móveis), alterações não programadas e/ou não autorizadas e alterações para remediar vulnerabilidades. Processos típicos para gerenciar mudanças de configuração em sistemas incluem, por exemplo, Placas de Controle de Configuração que aprovam mudanças propostas para sistemas. Para novos sistemas de informação de desenvolvimento ou sistemas submetidos a grandes atualizações, as organizações consideram a inclusão de representantes de organizações de desenvolvimento nos painéis de controle de configuração. A auditoria de mudanças inclui atividades antes e depois de mudanças nos sistemas de informações organizacionais e nas atividades de auditoria necessárias para implementar tais mudanças."
),
(50, 4, 4, 2, 1, --CM-4
"ANÁLISE DE IMPACTO DE SEGURANÇA",
"A organização analisa as mudanças que serão efetuadas no sistema para determinar potenciais impactos de segurança de serem implementadas.",
"Os funcionários da organização com responsabilidades de segurança da informação (por exemplo, Administradores do Sistema, Oficiais de Segurança do Sistema, Gerentes de Segurança do Sistema e Engenheiros de Segurança do Sistema) conduz análises de impacto de segurança. As pessoas que conduzem análises de impacto sobre a segurança possuem as habilidades e conhecimentos técnicos necessários para analisar as mudanças nos sistemas e as ramificações de segurança associadas. A análise de impacto de segurança pode incluir, por exemplo, a revisão dos planos de segurança para entender os requisitos de controle de segurança e a análise da documentação de design do sistema para entender a implementação do controle e como alterações específicas podem afetar os controles. As análises de impacto de segurança também podem incluir avaliações de risco para entender melhor o impacto das mudanças e determinar se controles de segurança adicionais são necessários. Análises de impacto de segurança são dimensionadas de acordo com as categorias de segurança dos sistemas."
),
(51, 4, 5, 1, 2, --CM-5
